#!/usr/bin/env python3
"""Run the high-dynamic phase-local speed training pipeline."""
...
GPU_INDEX = int(os.environ.get("HIGHDYN_GPU_INDEX", "6"))
...
TASKS = {
    "kick": "Tracking-Flat-T800-HighDyn-PhaseLocalKick3p2s-v0",
    "riot": "Tracking-Flat-T800-HighDyn-PhaseLocalRiot3p2s-v0",
}
SEEDS = {"kick": 2201, "riot": 2301}
LEADS = {"kick": 2, "riot": 1}
BASE_RUNS = {
    "kick": "2026-05-25_18-38-20_kick_speedlead_multispeed_seed2201",
    "riot": "2026-05-28_05-36-47_riot_speedlead_multispeed_seed2301",
}
BASE_CHECKPOINT = "model_75995.pt"
...
SCALAR_TAGS = (
    ...
    "Episode_Reward/phase_leg_lin_vel_projection",
    "Episode_Reward/phase_leg_ang_vel_projection",
    "Episode_Reward/phase_arm_lin_vel_projection",
    "Episode_Reward/phase_arm_ang_vel_projection",
    "Episode_Reward/phase_torso_ang_vel_projection",
    "Episode_Reward/phase_joint_vel",
    ...
    "Metrics/motion/phase_fast/error_local_lin_vel",
    "Metrics/motion/phase_fast/error_local_ang_vel",
    "Metrics/motion/phase_fast/error_joint_vel",
    "Metrics/motion/phase_fast/error_local_body_pos",
    "Metrics/motion/phase_fast/error_aux_lin_vel",
    "Metrics/motion/phase_fast/error_aux_ang_vel",
)

RELAXED_OVERRIDES = [
    "env.terminations.anchor_pos.params.threshold=0.70",
    "env.terminations.anchor_ori.params.threshold=1.40",
    "env.terminations.ee_body_pos.params.threshold=0.80",
]
SELECT_OVERRIDES = [
    "env.terminations.anchor_pos.params.threshold=0.65",
    "env.terminations.anchor_ori.params.threshold=1.35",
    "env.terminations.ee_body_pos.params.threshold=0.75",
    "env.rewards.action_rate_l2.weight=-0.035",
]
...
def selection_score(metrics: dict[str, float]) -> float:
    phase_lin = metrics.get(
        "Metrics/motion/phase_fast/error_local_lin_vel",
        metrics.get("Metrics/motion/error_body_lin_vel", 1.0e6),
    )
    phase_ang = metrics.get(
        "Metrics/motion/phase_fast/error_local_ang_vel",
        metrics.get("Metrics/motion/error_body_ang_vel", 1.0e6),
    )
    phase_joint = metrics.get(
        "Metrics/motion/phase_fast/error_joint_vel",
        metrics.get("Metrics/motion/error_joint_pos", 1.0e6),
    )
    body_pos = metrics.get(
        "Metrics/motion/phase_fast/error_local_body_pos",
        metrics.get("Metrics/motion/error_body_pos", 1.0e6),
    )
    early = (
    ...
    episode_length = metrics.get("Train/mean_episode_length", 0.0)
    length_penalty = max(0.0, 158.0 - episode_length)
    return 0.45 * phase_lin + 0.30 * phase_ang + 0.15 * phase_joint + 0.10 * body_pos + 10.0 * early + length_penalty
...
def train_stage(
    *,
    ...
    cmd = [
        ...
        "--checkpoint",
        checkpoint,
        f"env.actions.joint_pos.ref_advance_steps={LEADS[action]}",
        "env.scene.contact_forces.debug_vis=false",
        "env.commands.motion.debug_vis=false",
    ]
    cmd.extend(extra_overrides)
    run_cmd(cmd, pipeline_dir / f"{stage}_{run_name}.log", env=command_env())
...
def record_video(
    *,
    ...
    run_cmd(cmd, pipeline_dir / f"video_{stage}_{run_dir.name}_{label}.log", env=command_env())

    play_dir = run_dir / "videos" / "play"
    videos = sorted(play_dir.glob("*.mp4"), key=lambda path: path.stat().st_mtime)
    if videos:
        target = run_dir / "videos" / f"phasefast_{stage}_{label}.mp4"
...
def stage_record(run_dir: Path, checkpoint: str, metrics: dict[str, float]) -> dict[str, Any]:
    return {
...
def run_action(action: str, speed_files: list[Path], pipeline_dir: Path, state: dict[str, Any]) -> None:
    ...
    action_state: dict[str, Any] = {"selected_lead": LEADS[action], "base_run": BASE_RUNS[action]}
    state[action] = action_state
    write_state(pipeline_dir / "state.json", state)

    run_dir_a, checkpoint_a, metrics_a = train_stage(
        action=action,
        stage=f"{action}_stage_a",
        run_name=f"{action}_phasefast_a_seed{SEEDS[action]}",
        motion_file=source_motion,
        iterations=8000,
        resume_run=BASE_RUNS[action],
        checkpoint=BASE_CHECKPOINT,
        extra_overrides=RELAXED_OVERRIDES,
        pipeline_dir=pipeline_dir,
    )
    action_state["stage_a"] = stage_record(run_dir_a, checkpoint_a, metrics_a)
    write_state(pipeline_dir / "state.json", state)
    ...
    run_dir_b, checkpoint_b, metrics_b = train_stage(
        ...
        run_name=f"{action}_phasefast_b_multispeed_seed{SEEDS[action]}",
        motion_file=multispeed_motion,
        iterations=12000,
        resume_run=run_dir_a.name,
        checkpoint=checkpoint_a,
        extra_overrides=RELAXED_OVERRIDES,
        pipeline_dir=pipeline_dir,
    )
    action_state["stage_b"] = stage_record(run_dir_b, checkpoint_b, metrics_b)
    write_state(pipeline_dir / "state.json", state)
    for no_terminations in (False, True):
        record_video(
            action=action,
    ...
    run_dir_c, checkpoint_c, metrics_c = train_stage(
        ...
        run_name=f"{action}_phasefast_c_select_seed{SEEDS[action]}",
        motion_file=source_motion,
        iterations=6000,
        resume_run=run_dir_b.name,
        checkpoint=checkpoint_b,
        extra_overrides=SELECT_OVERRIDES,
        pipeline_dir=pipeline_dir,
    )
    action_state["stage_c"] = stage_record(run_dir_c, checkpoint_c, metrics_c)
    candidates = [action_state["stage_a"], action_state["stage_b"], action_state["stage_c"]]
    action_state["selected_final"] = min(candidates, key=lambda item: item["score"])
    write_state(pipeline_dir / "state.json", state)
    for no_terminations in (False, True):
        record_video(
            action=action,
...
def main() -> None:
    ...
    pipeline_dir = LOG_ROOT / f"phasefast_pipeline_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
